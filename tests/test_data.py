...
class TestTranslator(unittest.TestCase):
    ...
    def test_transtalte_prices_func(self):
        ...
        """
        prices = [
            PriceData(code="USD___", source="bonbast", price_high=1),
            PriceData(code="USD", source="bonbast", price_high=2),
            PriceData(code="USD", source="bonbast___", price_high=3),
        ]
        translated = translate_prices(prices=prices, prune=True)
        self.assertEqual(len(translated), 1)
        self.assertEqual(translated[0].price_high, 2)
        ...
        self.assertEqual(translated_no_prune[0].price_high, 1)
        self.assertEqual(translated_no_prune[2].price_high, 3)
